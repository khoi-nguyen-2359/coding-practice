 ...
 */
class Solution {
    fun buildTree(inorder: IntArray, postorder: IntArray): LcTreeNode? {
        return iterativelyCheck(inorder, postorder)
    }

    private fun iterativelyCheck(inorder: IntArray, postorder: IntArray): LcTreeNode? {
        val stack = Stack<Triple<LcTreeNode, IntArray, IntArray>>()
        val rootNode = postorder.lastOrNull()?.let(::LcTreeNode) ?: return null
        ...
        while (stack.isNotEmpty()) {
            ...
            val leftNode = leftPostOrd.lastOrNull()?.let(::LcTreeNode)
            ...
            val rightNode = rightPostOrd.lastOrNull()?.let(::LcTreeNode)
    ...
    private fun recursivelyCheck(inorder: IntArray, postorder: IntArray): LcTreeNode? {
        ...
        val parentNode = LcTreeNode(parent).apply {
            this.left = left
            this.right = right
        }
        return parentNode
...
fun main() {
    arrayOf(
        intArrayOf(1,2,3,5,4,7,6,8) to intArrayOf(1,2,5,7,8,6,4,3) to LcTreeNode.create(3,2,4,1,null,5,6,null,null,null,null,null,null,7,8),
        intArrayOf(9,3,15,20,7) to intArrayOf(9,15,7,20,3) to LcTreeNode.create(3,9,20,null,null,15,7),
        intArrayOf(9,3) to intArrayOf(9,3) to LcTreeNode.create(3,9),
